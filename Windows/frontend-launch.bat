::======================================================================
::  frontend-launch.cmd  –  Double-clickable launcher for TDC001 GUI
::  • Prompts to rebuild image only when one already exists
::  • Starts (or recreates) the container
::  • Opens  http://localhost:6080
::======================================================================

@echo off
REM --- Batch stub: feed the remainder of this file to PowerShell ----------
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$batch = Get-Content -LiteralPath '%~f0';" ^
    "$psStart = ($batch | Select-String -SimpleMatch '#===POWERSHELL===').LineNumber + 1;" ^
    "$ps = $batch[$psStart..($batch.Length-1)] -join [Environment]::NewLine;" ^
    "Invoke-Command ([ScriptBlock]::Create($ps))"
goto :eof

#===POWERSHELL===  (everything below runs INSIDE PowerShell) ##############
param()   # no CLI args today
...
try {
    ...
    Write-Host "[ERROR] Compose file not found." -Foreground Red
...
try { docker info | Out-Null }
    ...
    Write-Host "[ERROR] Docker Desktop is not running." -Foreground Red
    Read-Host "Press Enter to exit" | Out-Null
    exit 1
}
...
if ($imageId) {
    ...
    if ((Read-Host "Rebuild it? [y/N]") -match '^[Yy]') { $rebuild = $true }
} else {
    $rebuild = $true   # first run → must build
}
